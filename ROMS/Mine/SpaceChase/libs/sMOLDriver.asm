...
;; How long a row lasts in ticks (1 = one row per call to `sMOL_dosound`, etc. 0 translates to 256)
ticks_per_row: db
...
current_wave: db
def sMOL_NO_WAVE equ 100
    EXPORT sMOL_NO_WAVE

;; Everything between this and `end_zero` is zero-initialized by `sMOL_init`
start_zero:
...
SECTION "sMOL        - Sound Driver", ROM0
...
;;; Sets up sMOLDriver to play a song.
;;; !!! BE SURE THAT `sMOL_dosound` WILL NOT BE CALLED WHILE THIS RUNS !!!
;;; Param: HL = Pointer to the "song descriptor" you wish to load (typically exported by sMOLTracker).
;;; Destroys: AF C DE HL
sMOL_init::
    ld a, [sMOL_Bank]
    ld [$2000], a
...
fx_pos_jump:
    ...
    ld a, c         ; move the order ID into A
    cp $FF         ; compare A with 255
    call z, Audio_ResetChannels  ; if equal, jump to ResetChannels
